...
async function runRewarded() {
  ...
  try {
    ...
    const adShowPromise = AdMob.showRewardVideoAd();
    const timeoutPromise = new Promise((resolve) => {
      safetyWatchdog = setTimeout(() => {
        debugError("[ads] Watchdog forced ad completion/dismissal due to timeout", null);
        resolve(false);
      }, 25000);
    });
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    if (rewardedInFlight) {
      debugLog("[ads] App minimized during rewarded ad. Forcing flight release.");
